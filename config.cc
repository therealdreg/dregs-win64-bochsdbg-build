...
static int parse_line_formatted(const char *context, int num_params, char *params[])
{
  ...
  if (!strcmp(params[0], "#include")) {
    ...
    else if (params[1][8] > '0' && params[1][8] < '8'){
      bx_dbg.magic_break = params[1][8];
      bx_dbg.magic_break -= 0x30;
      if (1 == bx_dbg.magic_break) // backward compatibility hack, 1 --> bx
      {
        bx_dbg.magic_break = 3; // bx is regid 3
      } else if (3 == bx_dbg.magic_break) // 3 --> cx
      {
        bx_dbg.magic_break = 1; // cx is regid 1
      }
      BX_INFO(("Stopping on magic break points: %c (real %d)", params[1][8], bx_dbg.magic_break));
    }
    else {
      PARSE_ERR(("%s: magic_break directive malformed (valid values: 0-7).", context));
    }
#else
